...
IP_ADDRESS=$(ifconfig eth0 | grep -Eo 'inet (addr:)?([0-9]{1,3}\.){3}[0-9]{1,3}' | grep -v '127.0.0.1' | awk '{print $2; exit}' | sed 's/addr://')
#                      ^_______哪个网口获取分配的ip填哪个网口，通常是eth0.旁路由以及单网口机型或不用理会
